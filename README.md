...
- ✅ **PST 기간별 백업**: 연결된 PST 파일을 1년/2년/3년/4년/5년 단위로 백업 (대용량 PST 대응)
...
Outlook에 **추가로 연결된 PST 파일**의 받은편지함/보낸편지함을 **기간별**로 백업합니다.  
대용량 PST(예: 20GB)도 기간을 나눠 처리해 안정적으로 동작합니다.

1. **Alt + F8** (매크로 실행)
2. 아래 PST 매크로 중 하나 선택 후 **실행** 클릭
3. 백업 완료 후 결과 메시지 확인

| 매크로 이름 | PST 백업 기간 |
|------------|----------------|
| `BackupPSTLastYearMails` | 오늘 기준 이전 **1년치** |
| `BackupPSTLast2YearsMails` | 오늘 기준 이전 **2년치** |
| `BackupPSTLast3YearsMails` | 오늘 기준 이전 **3년치** |
| `BackupPSTLast4YearsMails` | 오늘 기준 이전 **4년치** |
| `BackupPSTLast5YearsMails` | 오늘 기준 이전 **5년치** |

**기능**:
- 현재 프로필에 연결된 **모든 PST 파일** 대상
- 각 PST의 **받은편지함/보낸편지함**만 해당 기간 메일만 백업 (날짜 필터 적용)
- 이미 저장된 메일은 자동으로 건너뜀
- PST가 없으면 안내 메시지 표시

**참고**:
- PST는 **파일 → 열기 및 내보내기 → Outlook 데이터 파일 열기**로 추가할 수 있습니다
- **20GB 이상 대용량 PST**는 1년치씩 여러 번 실행하는 것을 권장합니다
...
### Q: PST 파일로 불러온 메일도 백업할 수 있나요?
**A**: 네. Alt+F8에서 **PST 기간별 백업** 매크로(`BackupPSTLastYearMails`, `BackupPSTLast2YearsMails` 등 1~5년치)를 실행하면, 연결된 **모든 PST**의 받은편지함/보낸편지함을 해당 기간만큼만 백업합니다. 대용량 PST는 1년치씩 나눠 실행하는 것을 권장합니다.
